...
interface BillableLesson {
  id: string; date: string; duration_hours: number; billable_amount: number; student_id: string; student_name: string;
}
interface BillableFormula {
  id: string; offer_name: string; total_price: number; student_id: string; student_name: string;
}
interface PayerPreview {
  payer_id: string; payer_name: string; students: { id: string; name: string }[]; lessons: BillableLesson[]; formulas: BillableFormula[]; total_ht: number;
}
  ...
  const handleGenerate = async (preview: PayerPreview) => {
    ...
    try {
      ...
      for (const lesson of preview.lessons) {
        lines.push({ description: `Séance conduite – ${lesson.student_name} – ${formatDate(lesson.date)} (${lesson.duration_hours}h)`, quantity: 1, unit_price: lesson.billable_amount, total_ht: lesson.billable_amount, source_lesson_id: lesson.id });
      }
      for (const formula of preview.formulas) {
        lines.push({ description: `${formula.offer_name} – ${formula.student_name}`, quantity: 1, unit_price: formula.total_price, total_ht: formula.total_price, source_formula_id: formula.id });
      }
      ...
      const { data: invoice, error: invErr } = await supabase.from("invoices").insert({
        ...
        student_id: preview.students[0].id, payer_id: preview.payer_id, total_ht: totalHt, tva_amount: tvaAmount, total_ttc: totalTtc,
                ...
                <button onClick={() => setExpandedPayer(isExpanded ? null : preview.payer_id)} className="w-full flex items-center justify-between p-4 hover:bg-accent/30 transition-colors text-left">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground">{preview.students.length} apprenant{preview.students.length > 1 ? "s" : ""} · {preview.lessons.length} séance{preview.lessons.length > 1 ? "s" : ""} · {preview.formulas.length} forfait{preview.formulas.length > 1 ? "s" : ""}</p>
                    </div>
                  </div>
                  <div className="flex items-center gap-3">
                  ...
                  <div className="border-t border-border px-4 pb-4 pt-3 space-y-3">
                    <div className="flex flex-wrap gap-1.5">
                      {preview.students.map((s) => (
                        <span key={s.id} className="status-badge rounded-md bg-primary/10 text-primary"><Users className="w-3 h-3" /> {s.name}</span>
                      ))}
                    </div>
                    <div className="rounded-lg border border-border overflow-hidden">
                      <table className="w-full text-sm">
                        <thead><tr className="bg-muted/50"><th className="text-left px-3 py-2 text-xs font-medium text-muted-foreground">Description</th><th className="text-right px-3 py-2 text-xs font-medium text-muted-foreground">Montant HT</th></tr></thead>
                        <tbody>
                          {preview.lessons.map((l) => (<tr key={l.id} className="border-t border-border/50"><td className="px-3 py-2 text-foreground">Séance – {l.student_name} – {formatDate(l.date)} ({l.duration_hours}h)</td><td className="px-3 py-2 text-right font-medium text-foreground">{formatEur(l.billable_amount)}</td></tr>))}
                          {preview.formulas.map((f) => (<tr key={f.id} className="border-t border-border/50"><td className="px-3 py-2 text-foreground">{f.offer_name} – {f.student_name}</td><td className="px-3 py-2 text-right font-medium text-foreground">{formatEur(f.total_price)}</td></tr>))}
                        </tbody>
                        <tfoot><tr className="border-t border-border bg-muted/30"><td className="px-3 py-2 font-semibold text-foreground">Total HT</td><td className="px-3 py-2 text-right font-bold text-foreground">{formatEur(preview.total_ht)}</td></tr></tfoot>
                      </table>
                    </div>
                    {!isGenerated && (
