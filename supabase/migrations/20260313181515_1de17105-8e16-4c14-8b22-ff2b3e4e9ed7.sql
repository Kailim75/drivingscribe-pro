...
-- Admin function to generate/regenerate API key for any org
CREATE OR REPLACE FUNCTION public.admin_generate_api_key(_org_id uuid)
 RETURNS text
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  UPDATE organizations SET webhook_api_key = _key, updated_at = now() WHERE id = _org_id;
