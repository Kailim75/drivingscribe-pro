-- Lot 3 — Fiabilité des paiements.
-- 1) Les totaux de facture (payé / reste dû / statut) étaient recalculés par le navigateur
--    en plusieurs requêtes : une coupure réseau au milieu laissait des totaux faux.
--    Un trigger en base rend le recalcul automatique et infaillible.
-- 2) Les factures publiques étaient accessibles par leur simple identifiant technique :
--    ajout d'un jeton aléatoire dédié au partage.

-- ── 1. Recalcul des totaux de facture ──────────────────────────────────────────
...
BEGIN
  ...
  ELSIF _current_status IN ('payé', 'partiellement_payé') THEN
    -- Plus aucun paiement (suppression) : on ne peut pas retrouver l'état antérieur exact,
    -- « envoyé » est le seul état cohérent pour une facture émise sans encaissement.
    _new_status := 'envoyé';
  ELSE
...
-- Remise d'aplomb des factures dont les totaux auraient déjà dérivé.
DO $$
DECLARE
  inv record;
BEGIN
...
-- ── 2. Jeton public de partage des factures ────────────────────────────────────
